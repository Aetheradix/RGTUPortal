...
const ActionAnukampaAppointment: React.FC = () => {
  ...
  return (
    <PageLayout title="Director Anukampa Application Approval - Action On Anukampa Appointment">
      ...
      {selectedApp && (
        <div ref={detailRef} className="space-y-6 pb-10 animate-fade-in">
          <h2 className="text-xl font-bold text-blue-800 border-b-2 border-blue-200 pb-2">Register appropriate action on compassionate application at Director level</h2>
          ...
          <div className="grid grid-cols-1 gap-6">
              ...
              <DataTable value={familyMembers} className="p-datatable-sm" showGridlines stripedRows sortMode="multiple">
                <Column field="srNo" header="Sr.No." sortable style={{ width: '4rem' }} />
            ...
            <Card title="Details of Applicant Documents">
              <DataTable value={documents} className="p-datatable-sm" showGridlines stripedRows sortMode="multiple">
                <Column field="srNo" header="Sr. No" sortable style={{ width: '4rem' }} />
                <Column field="docName" header="Document Name" sortable />
                <Column
                  header="View"
                  body={() => (
                    <Button icon="pi pi-file-pdf" className="p-button-text p-button-danger p-0" />
                  )}
                />
              </DataTable>
            </Card>
          ...
          <Card title="Final Action" className="bg-blue-50 border-2 border-blue-200">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              <div className="flex flex-col gap-2">
                <label className="font-bold text-blue-900">Action*</label>
                <Dropdown 
                  value={actionValue} 
                  options={[
                    { label: "Objection/Liability in Application", value: "objection" },
                    { label: "Accept Application", value: "accept" },
                    { label: "Forward to Divisional Director", value: "fwd_director" },
                    { label: "Forward to DEO", value: "fwd_deo" }
                  ]} 
                  onChange={(e: DropdownChangeEvent) => setActionValue(e.value)} 
                  placeholder="Select Action" 
                  className="w-full shadow-sm"
                />
              </div>
              <div className="flex flex-col gap-2">
                 <label className="font-bold text-blue-900">Remarks</label>
                 <InputTextarea rows={2} placeholder="Remarks..." className="w-full" />
              </div>
              <div className="md:col-span-2 flex justify-end">
                <Button label="Submit Action" icon="pi pi-check-circle" className="p-button-success p-button-lg px-10 shadow-lg" />
              </div>
            </div>
          </Card>
        </div>
...
export default ActionAnukampaAppointment;
