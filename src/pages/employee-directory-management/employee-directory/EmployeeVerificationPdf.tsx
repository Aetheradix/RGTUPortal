...
import {
  ...
  districtOptions,
  blockOptions,
} from "./data"; // Assuming standard options are shared
import { pdfVerificationData } from "./data";
import { getPdfVerificationColumns } from "./table";
import { InputText } from "primereact/inputtext";

const EmployeeVerificationPdf: React.FC = () => {
  ...
  const onDownloadPdf = (data: any) => {
    toast.current?.show({
      severity: "info",
      summary: "Downloading",
      detail: `Preparing PDF for ${data.empUniqueId}`,
      life: 2000,
    });
  };

  return (
    <PageLayout title="Employee Verification Data(PDF)">
      <Toast ref={toast} />

      {/* Search Section (Capture25.PNG) */}
        ...
        <div className="bg-white p-6 rounded shadow-sm border border-gray-100 mt-6 animate-fade-in">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-md font-bold text-gray-700">
              Employee Verification Data (PDF)
            </h3>
          ...
          <Table
            ...
            columns={getPdfVerificationColumns(onDownloadPdf)}
            showPagination={true}
            rowsPerPage={10}
...
export default EmployeeVerificationPdf;
