	...
	const renderSubmodule = (submodule: SearchResult) => {
		...
		return (
			<div key={submodule.route}>
				...
				>
					<span className="text-sm text-gray-800 flex-1">{submodule.title}</span>
					<span className={`text-xs px-2 py-0.5 rounded-full ${getTypeColor(submodule.type)} shrink-0`}>
						{submodule.type}
					</span>
						...
						{submodule.children.map((page: SearchResult) => (
							...
							>
								<div className="flex items-center gap-2">
									...
									<span className={`text-xs px-2 py-0.5 rounded-full ${getTypeColor(page.type)} shrink-0`}>
										{page.type}
									</span>
								</div>
								<p className="text-xs text-gray-500 mt-0.5">{page.breadcrumb}</p>
							</button>
						))}
					</div>
				)}
			...
			return (
				<div key={item.route} className="border-b border-gray-200">
					...
					<div className="px-4 py-2.5 bg-gray-50 flex items-center gap-2 border-b border-gray-200">
						...
						<span className={`text-xs px-2 py-0.5 rounded-full ${getTypeColor(item.type)} shrink-0 ml-auto`}>
							{item.type}
						</span>
					</div>
					...
					{item.children && item.children.length > 0 && (
						<div className="bg-white">
							{item.children.map((submodule: SearchResult) => renderSubmodule(submodule))}
						</div>
					)}
				</div>
			);
	...
	return (
		<div ref={searchRef} className="relative flex-1 max-w-2xl">
			...
			<input
				...
				onFocus={() => debouncedQuery.trim().length >= 2 && setShowSearchResults(true)}
